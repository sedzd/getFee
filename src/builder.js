module.exports.builder = (() => {
  ...
  const formatFeeStructure = feeStructure => feeStructure
    .reduce((acc, curr) => {
      acc[curr['type']] = curr
      return acc
    }, {})
  ...
  const applyMicropayments = ({ amount }) => ({ tpvTier, TIERING_MICROPAYMENTS_PERCENTAGE: tmp = null, ...rest }) => ({
  ...
  const getTierByCount = ({ count }) => ({ TIERING_COUNT: tCount = null, ...rest }) => ({
    ...(tCount
      ? {
  ...
  const applyFlatTier = () => ({ countTier, TIERING_FLAT: tFlat = null, ...rest }) => ({
    ...(countTier >= 0 && tFlat
      ? {
        countTier,
  ...
  const applyPerecentTier = ({ amount }) => ({ countTier, TIERING_PERCENTAGE: tPerecentage = null, fee = 0, ...rest }) => ({
  ...
  const applyTaxWithHold = () => ({ TAX_WITHOLDING: wh = null, fee, ...rest }) => ({
    ...(wh && fee > 0
      ? {
        fee,
        withHold: wh['fee'] * fee,
